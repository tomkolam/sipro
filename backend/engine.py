...
async def process_lead_capture(provider: str, payload: dict, org_id: str = ORG_ID,
    ...
    attribution = {
        "adset_id": payload.get("adset_id"), "ad_id": payload.get("ad_id"),
        "creative_id": payload.get("creative_id"), "form_id": payload.get("form_id"),
    }
    lead = {
        "id": lead_id, "org_id": org_id, "name": payload.get("name") or "Lead Baru",
        "phone": phone, "email": payload.get("email"), "source": src,
        "campaign": payload.get("campaign"), "stage": "acquisition", "assigned_to": assignee,
    ...
    await db.lead_capture_events.insert_one({
        ...
        "status": "processed", "lead_id": lead_id, "source": src, "campaign": payload.get("campaign"),
...
async def _h_lead_captured(ev):
    ...
    # CAPI feedback loop: tell the ad platform we received a qualified Lead.
    try:
        await record_conversion(event_name="Lead", lead=lead, org_id=org)
    except Exception:  # noqa: BLE001
        logger.exception("CAPI 'Lead' feedback failed for %s", ev.get("entity_id"))
...
async def _h_deal_booked(ev):
    ...
    # CAPI feedback loop: booking = mid-funnel conversion (InitiateCheckout).
    try:
        lead = await db.leads.find_one({"id": deal.get("lead_id")}, {"_id": 0}) if deal.get("lead_id") else None
        await record_conversion(event_name="InitiateCheckout", lead=lead, deal=deal,
                                value=deal.get("price", 0), org_id=org)
    except Exception:  # noqa: BLE001
        logger.exception("CAPI booking feedback failed for deal %s", deal.get("id"))


HANDLERS["deal.booked"] = [_h_deal_booked]


async def _h_deal_sold(ev):
    """On AJB/sold: feed a final 'Purchase' conversion back to the ad platform."""
    deal = await db.deals.find_one({"id": ev["entity_id"]}, {"_id": 0})
    if not deal:
        return
    org = ev.get("org_id", ORG_ID)
    try:
        lead = await db.leads.find_one({"id": deal.get("lead_id")}, {"_id": 0}) if deal.get("lead_id") else None
        await record_conversion(event_name="Purchase", lead=lead, deal=deal,
                                value=deal.get("price", 0), org_id=org)
    except Exception:  # noqa: BLE001
        logger.exception("CAPI 'Purchase' feedback failed for deal %s", deal.get("id"))
...
HANDLERS["deal.sold"] = [_h_deal_sold, _h_deal_won]
