...
# ----------------------------- Units -----------------------------
@router.get("/units")
async def list_units(project_id: str = None, status: str = None, skip: int = 0, limit: int = 200,
    ...
    rows = await db.units.find(q, {"_id": 0}).sort("code", 1).skip(skip).limit(limit).to_list(limit)
    ...
    for st in ("available", "reserved", "booked", "sold"):
        counts[st] = await db.units.count_documents({**q, "status": st})
    return {"data": serialize_doc(rows), "total": total, "counts": counts}


# ----------------------------- Deals -----------------------------
@router.get("/deals")
async def list_deals(status: str = None, skip: int = 0, limit: int = 50,
    ...
    rows = await db.deals.find(query, {"_id": 0}).sort("created_at", -1).skip(skip).limit(limit).to_list(limit)
    # enrich
    for d in rows:
