...
@router.get("")
async def list_customers(q: str = None, skip: int = 0, limit: int = 50,
    ...
    rows = await db.customers.find(query, {"_id": 0}).sort("created_at", -1).skip(skip).limit(limit).to_list(limit)
