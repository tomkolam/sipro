...
2. Fee otomatis muncul pada trigger, nominal **sama** dengan hitungan aturan (uji beberapa basis: percent, fixed, tier).
3. Tidak ada fee tanpa aturan yang berlaku (INV-09).
4. Saldo `2-1500` tetap = Σ (netto − terbayar) setelah semua uji (invarian lama tidak rusak).
5. Halaman Mitra: tabel + profil mitra (tab: Profil, Kontrak & Dokumen, Aturan Fee, Lead, Tagihan Fee, Analitik).
6. Gate baru `verify_partner.py` uji-mutasi + `run_all_gates.sh` tetap PASS.
