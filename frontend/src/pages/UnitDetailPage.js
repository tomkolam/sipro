    ...
    <div data-testid={MASTERPLAN.unitPage} className="space-y-5">
      ...
      <Tabs defaultValue="summary" className="space-y-4">
        ...
        <TabsContent value="build">
          <div className="rounded-lg border bg-card p-4">
            <h2 className="mb-2 font-heading text-base font-semibold">Pembangunan</h2>
            <Row label="Status pembangunan"
              value={<StatusPill status={u.construction_status} group="construction_status" />} />
            <Row label="Progres terverifikasi" value={`${u.construction_progress || 0}%`} />
            <Row label="Jadwal unit" value={data?.schedule
              ? `${data.schedule.template_name || "Template"} · ${(data.schedule.items || []).length} langkah`
              : "belum dijadwalkan"} />
            <Row label="Mulai bangun butuh DP terbayar"
              value={data?.settings?.["build.require_dp_before_start"] ? "Ya" : "Tidak"} />
          </div>
        </TabsContent>

        <TabsContent value="docs" className="space-y-3">
          <div className="rounded-lg border bg-card p-4">
            <h2 className="mb-2 font-heading text-base font-semibold">Perizinan unit</h2>
            {(data?.permits || []).length ? (
              <ul className="space-y-1 text-sm">
                {data.permits.map((p) => (
                  <li key={p.id} className="flex items-center justify-between border-b py-1">
                    <span>{p.permit_type || p.type} · {p.number || "tanpa nomor"}</span>
                    <StatusPill status={p.status} group="permit_status" />
                  </li>
                ))}
              </ul>
            ) : (
              <EmptyState title="Belum ada izin"
                description="Perizinan kini menempel pada unit/proyek, bukan menu terpisah." />
            )}
          </div>
          <div className="rounded-lg border bg-card p-4">
